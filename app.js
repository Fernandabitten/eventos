...
function verifyJWT(req, res, next) {
  ...
  jwt.verify(token, SECRET, function (err, usersDB) {
    if (err) {
      ...
      if (savedUser) {
        ...
        if (savedUser.hash == hash) {
          ...
          const authToken = jwt.sign(
            { username: username, usertype: savedUser.usertype, sessionToken },
            SECRET,
            { expiresIn: 300 }
          );
          res.cookie("authToken", authToken, { auth: true });
    ...
    readFile(EVENTS_DIR)
      .then(async (eventsDB) => {
        ...
        const qrcodeHash = crypto
          .createHmac("sha1", session.username)
        ...
        if (
          ...
          eventsDB[eventIndex].users.push({
            ...
            qrcode: `http://${HOSTNAME}${SERVER_PORT}/check-qrcode/${
              session.username
            }/${eventsDB[eventIndex].id}/${qrcodeHash.substring(0, 10)}`,
          });
        }
        saveFile(EVENTS_DIR, eventsDB).then((result) => res.send("OK"));
      })
...
app.get("/log-out", (req, res) => {
  ...
  SESSIONS = session;
  res.clearCookie("session_id", "authToken");
  res.clearCookie("authToken");
        ...
        res.send({
          ...
          htmlText: `
            <div id="event-confirmation-box" class="confirm-events">
              ...
              <button class="button" id="sair">Voltar</button>
            </div>
          `,
          exists: true,
